...
| BUG-023 | Critical | Profile save missing phoneNumber and custom semester mapping caused loss of data or 500 error on update; re-login avatar overwrite | `UpdateProfileRequest.java`, `StudentService.java`, `AuthService.java`, `authStore.ts`, `SettingsPage.tsx`, `AuthProvider.tsx` | **Fixed 2026-08-27** | Yes |
| BUG-024 | High | Materials subject filter hid uploaded files when filtered by subject because backend DTO omitted top-level `subjectId` and frontend lacked mapper/subjectId on material cards | `MaterialResponse.java`, `MaterialService.java`, `MaterialController.java`, `MaterialRepository.java`, `api.types.ts`, `materials.api.ts`, `materials/page.tsx`, `MaterialCard.tsx` | **Fixed 2026-08-27** | Yes |
| BUG-025 | Critical | Timetable truncated to 7 days despite 14+ day exam deadline; calendar omitted month headers & collapsed multi-week dates; slot cards omitted full start-end times; lacked study details modal & material topic traceability; past missed sessions overwritten on reschedule | `TimetableService.java`, `MaterialTopicReader.java`, `SlotResponse.java`, `api.types.ts`, `SlotDetailModal.tsx`, `timetable/page.tsx`, `timetable.module.css`, `SlotCard.tsx` | **Fixed 2026-08-28** | Yes |
| BUG-026 | High | Dashboard displayed "STUDY HOURS: 0 sessions" for scheduled pending sessions because `studyHours` was assigned to `totalCompleted` count | `frontend/src/app/(dashboard)/dashboard/page.tsx`, `frontend/src/utils/dashboardStats.ts`, `mobile/src/screens/dashboard/DashboardScreen.tsx`, `mobile/src/utils/dashboardStats.ts` | **Fixed 2026-08-31** | Yes |
| BUG-027 | High | Timetable study-window banner showed stale duration after changing Settings due to missing React Query cache update; future sessions could be toggled complete early | `SettingsPage.tsx`, `timetable/page.tsx`, `dateHelpers.ts`, `SlotCardItem`, `SlotDetailModal.tsx`, `TimetableService.java` | **Fixed 2026-08-31** | Yes |
| BUG-028 | Medium | Next.js API catch-all proxy passed 0-byte ArrayBuffer with `content-length` header on empty POST requests, triggering potential gateway / header mismatch errors | `frontend/src/app/api/[...path]/route.ts`, `frontend/src/api/videoRecommendations.api.ts` | **Fixed 2026-09-06** | Yes |